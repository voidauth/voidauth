import { Component, inject, input, model, type OnInit, ChangeDetectionStrategy } from '@angular/core'
    ...
    this.password()
      .valueChanges.pipe(
        ...
        switchMap((v) => {
          ...
          return from(this.userService.passwordStrength(v)).pipe(catchError(() => of({ score: -1 })))
        }),
      )
      .subscribe(({ score }) => {
        this.score = score + 1
