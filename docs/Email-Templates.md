...
> [!WARNING]
> Modified email templates **MUST** be renamed by removing the `.default` suffix from the file extension, as all `*.default.ejs` templates are overwritten with the current default template on each start of VoidAuth.
