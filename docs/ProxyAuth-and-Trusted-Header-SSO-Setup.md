...
> [!CAUTION]
> To determine real client IP addresses for rate limiting and logging, VoidAuth relies on standard X-Forwarded-* HTTP headers passed by your reverse proxy (e.g., Caddy, NGINX, Traefik). By default, `TRUSTED_PROXIES` is pre-configured to trust all private IP ranges, which should work for most deployments. For additional security, you *should* restrict `TRUSTED_PROXIES` to the exact IP address or CIDR range of your upstream reverse proxy. It is vital that you configure your reverse proxy to completely strip or overwrite incoming X-Forwarded-* headers from untrusted public clients before forwarding the request to VoidAuth. Never expose the application's listening port directly to the public internet, and when possible make your VoidAuth instance reachable only by your reverse proxy.
