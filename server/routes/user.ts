...
import { checkLoggedIn, validate } from "../util/validator"
...
import { emailValidation, nameValidation, stringValidation, usernameValidation } from "../util/validators"
...
userRouter.patch("/email",
  ...
  async (req: Request, res: Response) => {
    ...
    if (appConfig.EMAIL_VERIFICATION && email) {
      createEmailVerification(user, email)
    } else {
